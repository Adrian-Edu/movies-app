...
export default function Input() {
  ...
  const usersPassword = users.map((item) => item.password);
  ...
  const onForward = (e) => {
    if (userEmail === "") {
      ...
      setFormState({ ...formState, emailValidation: true });
    } else if (!usersEmail.includes(userEmail)) {
      setFormState({ ...formState, emailValidation: true });
  ...
  const changeState = () => {
    if (
      ...
      usersPassword.includes(userPassword)
    ) {
      logIn();
    }
  };
  ...
  useEffect(() => {
    if (userEmail === "" || !usersEmail.includes(userEmail)) {
        ...
        <div className="bg-yellow-100  w-4/5 md:w-5/12 xl:w-3/12 rounded-md">
          ...
          <form>
            <div>
              ...
              {userEmail.length > 11 && !usersEmail.includes(userEmail) ? (
                <div>
                  <span style={{ color: `red` }}>
