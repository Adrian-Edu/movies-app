...
export default function Input() {
  ...
  const usersPassword = users.map((item) => item.email);
...
export default function Input() {
  ...
  const onForward = (e) => {
    ...
    if (formState.isValid && userPassword === "") {
      setPasswordValidation(true);
    } else if (formState.isValid && userPassword.length < 10) {
      setPasswordValidation(true);
    } else {
      setPasswordValidation(false);
    }
  ...
  const changeState = () => {
    if (users.some(user => user.email === userEmail && user.password === userPassword)) {
      logIn();
    }
  };

  useEffect(() => {
    if (userEmail === "" || !users.some(user => user.email === userEmail)) {
      setFormState({ ...formState, isValid: false });
    }
        ...
        <div className="bg-yellow-100  w-4/5 md:w-5/12 xl:w-3/12 rounded-md">
          ...
          <form>
            <div>
              ...
              {userEmail.length > 11 && !users.some(user => user.email === userEmail) ? (
                <div>
                  <span style={{ color: `red` }}>
                    There is no account with this email address.
                  </span>
            ...
            {formState.isValid ? (
              <div className="pt-1 flex justify-center items-center flex-col ">
                ...
                {userPassword.length > 9 &&
                !users.some(user => user.password === userPassword) ? (
                  <div className="pt-2">
                    <span style={{ color: `red` }}>
                      The password you have provided is invalid!
                    </span>
          ...
          {formState.isValid &&
          userPassword.length > 10 &&
          users.some(user => user.email === userEmail && user.password === userPassword) ? (
            <>
              <Link href="/">
                <button
                  onClick={changeState}
